...
# 安装安卓和cordova插件
docker run --rm -i -t -v /e/gzphrm-app:/appDir app:v1 /bin/bash -c "source /etc/profile && cd /appDir && cordova platform add android@8.1.0 && cordova plugin add cordova-plugin-android-permissions@1.1.2 && cordova plugin add cordova-plugin-splashscreen@6.0.0 && cordova plugin add cordova-plugin-whitelist@1.3.5 && cordova plugin add cordova-hot-code-push-plugin@1.5.3"
```
完成以上步骤后，后续就可以使用命令行构建APP了：
> ```
> docker run --rm -i -t -v /e/gzphrm-app:/appDir app:v1 /bin/bash -c "source /etc/profile && cd /appDir && cordova build android && npm run copy-app"
> ```
